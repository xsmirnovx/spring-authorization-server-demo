...
//        )
    ...
    fun jwtCustomizer(): OAuth2TokenCustomizer<JwtEncodingContext> {
        return OAuth2TokenCustomizer { context: JwtEncodingContext ->
            ...
            Optional.of(context)
                .filter { ctx: JwtEncodingContext -> ctx.tokenType == OAuth2TokenType.ACCESS_TOKEN }
                .map<Any> { obj: JwtEncodingContext -> obj.getPrincipal() }
                .map { obj: Any? -> Authentication::class.java.cast(obj) }
                .map { p: Authentication ->
                    p.authorities.stream()
                        .map { obj: GrantedAuthority -> obj.authority }
                        .collect(Collectors.toSet())
                }
                .ifPresent { authorities: Set<String>? ->
